...
Parse.Cloud.define("eBayCategorySearch", function(request, response) {
    ...
    Parse.Cloud.httpRequest({
        ...
        params: {
            ...
            'SECURITY-APPNAME': 'eBay API key goes here',
            'GLOBAL-ID': 'EBAY-US',
...
Parse.Cloud.define("userCategorySave", function(request, response) {
    ...
    newUserCategory.save({
        ...
        error: function() {
            console.log('error!!!');
            response.error('Request failed b/c:' + response);
        }
    });
});
...
Parse.Cloud.define("addToMatchCenter", function(request, response) {
    ...
    console.log('just tryna add ish');
                    ...
                    if (results[i].get('itemLocation') == 'US') {
                        ...
                        var httpRequestPromise = Parse.Cloud.httpRequest({
                            ...
                            params: {
                                ...
                                //                    'affiliate.trackingId': 'eBay Affiliate Tracking ID goes here',
...
Parse.Cloud.define("editMatchCenter2", function(request, response) {
    ...
    var demOriginalSearchTerm = request.params.originalSearchTerm;
    var demCurrentUser = Parse.User.current();
    
    console.log('demOriginalSearchTerm:' + demOriginalSearchTerm);
    console.log('demCurrentUser:' + demCurrentUser);
...
//        response.success("mcItemsCount completed successfully.");
        ...
        // Set the job's error status
//        response.error('DAMN IT MAN');
        status.error("mcItemsCount FAILED");
    });                       
